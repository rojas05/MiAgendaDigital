...
class FragmentTrabajos : Fragment() {
    ...
    private fun insertar(item: entidadEgresos) {
        val egreso= entidadEgresos(null,item.fecha,item.costo,item.descripcion,"Trabajo",item.finca)
        CoroutineScope(Dispatchers.IO).launch{
    ...
    private suspend fun dates(){
        CoroutineScope(Dispatchers.IO).launch{
            insumosL = DB.getInstancia(requireContext()).DaoEgresos().getTrabajos()
    ...
    fun onItemSelected(item: entidadEgresos){
